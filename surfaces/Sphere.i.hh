//---------------------------------*-C++-*-----------------------------------//
/*!
 * \file surfaces/Sphere.i.hh
 * \brief Sphere inline method definitions
 ...
 */
Sphere::Sphere(const real_type* coeff)
    : origin_(coeff[0], coeff[1], coeff[2]), radius_sq_(coeff[3])
{
}
 ...
 */
SignedSense Sphere::calc_sense(const Real3& pos) const
{
    using Axis::x;
    using Axis::y;
    using Axis::z;
    const real_type x = pos[X] - origin_[X];
    const real_type y = pos[Y] - origin_[Y];
    const real_type z = pos[Z] - origin_[Z];
 ...
 */
void Sphere::calc_intersections(const Real3& pos,
                                const Real3& dir,
                                bool         on_surface,
 ...
 */
Real3 Sphere::calc_normal(const Real3& pos) const
{
